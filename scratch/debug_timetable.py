...
teachers = [
    {"id": t.id, "max_weekly": t.max_weekly_periods, "max_daily": t.max_daily_periods, "is_class_teacher_of": t.class_teacher_of.id if t.class_teacher_of else None}
...
def test_run(constraints_to_skip):
    generator = TimetableGenerator(
        teachers=teachers, classes=classes, divisions=divisions, subjects=subjects, teacher_subjects=teacher_subjects,
        days=school_setting.working_days, periods_per_day=school_setting.number_of_periods,
        lunch_break_period=school_setting.lunch_break_period, weekly_schedule=school_setting.weekly_schedule,
        global_max_weekly_teacher_periods=school_setting.max_weekly_teacher_periods
    )
    
    # We will hack the generator by removing code for skipped constraints, but since we can't easily,
    # let's just use it and see what happens if we patch it.
    pass
